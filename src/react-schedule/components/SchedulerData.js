...
export default class SchedulerData {
  ...
  canZoomIn() {
    ...
    return nextZoomLevel > this.config.zoomLevel && this.viewType !== ViewType.Custom && this.viewType !== ViewType.Week;
  }
    ...
    if (this.viewType !== viewType || this._shouldReloadViewType) {
      ...
      if (viewType === ViewType.Custom || viewType === ViewType.Custom1 || viewType === ViewType.Custom2) {
        ...
        if (this.viewType < viewType ||
          (this.viewType >= ViewType.Custom && viewType < ViewType.Custom)) {
  ...
  getContentCellWidth() {
    ...
    if (this.config.zoomEnabled && this.config.zoomLevel) {
      ...
      if (this.config.zoomLevel < 1.0 && this.headers.length && this.viewType !== ViewType.Year) {
  ...
  getDateLabel() {
    ...
    const currentView = this.config.views.find(view =>
      view.viewType === this.viewType &&
      view.showAgenda === this.showAgenda &&
      view.isEventPerspective === this.isEventPerspective
    );

    if (currentView && currentView.viewName === 'Day') {
      dateLabel = start.format('MMM D, YYYY');
    } else if (this.behaviors.getDateLabelFunc) {
  ...
  _handleRecurringEvents() {
    ...
    recurringEvents.forEach(item => {
      ...
      const windowStartDate = windowStart && windowStart.toDate ? windowStart.toDate() : new Date(windowStart);
      const windowEndDate = windowEnd && windowEnd.toDate ? windowEnd.toDate() : new Date(windowEnd);
      ...
      if (Number.isNaN(windowStartDate.getTime()) || Number.isNaN(windowEndDate.getTime())) {
        console.warn('Skipping recurring events due to invalid window boundaries', windowStartDate, windowEndDate);
        return;
      }
  ...
  _createHeaders() {
    ...
    if (this.showAgenda) {
      ...
      this._createWeekHeaders(headers, start, end);
  ...
  // For Shift View
  _createWeekHeaders(headers, start, end) {
    const shifts = this.config.shiftSlots;
    ...
    while (currentDay <= end) {
      ...
      if (this.config.displayWeekend || (dayOfWeek !== 0 && dayOfWeek !== 6)) {
        ...
        createWeekHeaders(currentDay, shifts);
      }
  ...
  _createInitHeaderEvents(header) {
    ...
    if (this.showAgenda) {
      ...
      if (this.viewType === ViewType.Week) {
        const end = this.localeDayjs(new Date(header.end));
        endValue = end.format(DATETIME_FORMAT);
      } else {
        endValue = start.add(incrementUnit === 'minutes' ? this.config.minuteStep : 1, incrementUnit)
  ...
  _getSpan(startTime, endTime, headers) {
    ...
    } else if (this.viewType === ViewType.Week || this.viewType === ViewType.Month || this.viewType === ViewType.Quarter || this.viewType === ViewType.Year || this.viewType === ViewType.Custom1) {
      if (this.viewType === ViewType.Week) {
        let overlapped = 0;
        for (let h of headers) {
          const hStart = new Date(h.start);
          const hEnd = h.endTime ? new Date(h.endTime) : new Date(h.end);
          if (hEnd > eventStart && hStart < eventEnd) overlapped++;
        }
        span = overlapped;
        if (span < 1) span = 1;
